...
const getkeyboardbyfilter = async (req, res) => {
  try {
      // const  category  = req.params;
      const { minPrice, maxPrice, minRating, maxRating  , subcategory , Switch , Connectivity} = req.query;
...
const addkeyboard = async (product) =>{
  try{
    const newkeyboard = product;
    if(!newkeyboard.switch || !newkeyboard.connectivity){
      throw new Error("Data is not complete");

    }
    const addp = new Product({newkeyboard});
...
export default { getkeyboardbyfilter , addkeyboard};
